...
import { PatientListResponse, Patient } from '../models/Patient';
...
export class PatientController {
  ...
  static async getPatientList(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.log('[PATIENT CONTROLLER] Successfully received a Patient list');
      res.json({ patients: formattedPatients });
    } catch (error) {
      console.error('[PATIENT CONTROLLER] Error retrieving Patient list:', error);
      res.status(500).json({
